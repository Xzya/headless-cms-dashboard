...
const UserTitle = translate(({ record, translate }) => (
    <span>
        {record ? translate('users.edit.title', { title: record.name }) : ''}
    </span>
));
...
const UserCreateToolbar = ({ permissions, ...props }) => (
    <Toolbar {...props}>
        <SaveButton
            label="users.action.save_and_show"
            redirect="show"
            submitOnEnter={true}
        />
        {permissions === 'admin' && (
            <SaveButton
                label="users.action.save_and_add"
                redirect={false}
                submitOnEnter={false}
                raised={false}
            />
    ...
    <Edit title={<UserTitle />} {...props}>
        {permissions => (
            <TabbedForm defaultValue={{ role: 'user' }}>
                <FormTab label="users.form.summary">
                    {permissions === 'admin' && <DisabledInput source="id" />}
                    <TextInput source="name" validate={required} />
                </FormTab>
                {permissions === 'admin' && (
                    <FormTab label="users.form.security">
                        <TextInput source="role" validate={required} />
                    </FormTab>
                )}
            </TabbedForm>
...
export const UserShow = ({ ...props }) => (
    <Show title={<UserTitle />} {...props}>
        {permissions => (
            <TabbedShowLayout>
                <Tab label="users.form.summary">
                    <TextField source="id" />
                    <TextField source="name" />
                </Tab>
                {permissions === 'admin' && (
                    <Tab label="users.form.security">
